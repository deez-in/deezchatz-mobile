...
This repository is the actual app that users install. It is the top-level consumer in the Nijhum ecosystem:

```mermaid
graph TD
    subgraph "Nijhum Mobile (this repo)"
    ...
    subgraph "Backend"
        API["Nijhum API<br/><small>REST + RMQTT</small>"]
    end
...
| **Auth Native Module** | [expo-google-native-oauth](https://github.com/nijhum-in/expo-google-native-oauth) |
...
2. **Registration**: The app signs these keys (VXEdDSA) and uploads them to the Nijhum API.
...
The server-side infrastructure uses **RMQTT** as the MQTT broker, secured with TLS for all client connections. Messages are relayed in real-time over MQTT. The Nijhum API also exposes a REST interface on port 3000 for registration and key discovery, and uses a private port 3001 to receive offline message webhooks from RMQTT for FCM push notifications.
