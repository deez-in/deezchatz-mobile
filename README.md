...
The server-side infrastructure uses **RMQTT** as the MQTT broker, secured with TLS for all client connections. MQTT connections are authenticated dynamically using a cryptographic signature generated by the device's Signed Prekey (preKey) to prove identity upon connection. Messages are relayed in real-time over MQTT. The DeezChatz API also exposes a REST interface on port 3000 for registration and key discovery, and uses a private port 3001 to receive offline message webhooks from RMQTT for FCM push notifications.
