...
import { getContactsPermissionStatus, requestContactsPermission } from "@/src/utils/helpers/contacts";
...
export default function ContactsPermission() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (status === "granted") {
        if (isMounted) {
          useSession.getState().markOnboardingCompleted();
          router.replace("/");
        }
      } else {
    ...
    <SafeAreaView style={dynamicStyles.container}>
      <View style={dynamicStyles.card}>
        <View style={dynamicStyles.iconContainer}>
          <Ionicons name="people" size={40} color={colors.onPrimaryContainer as string} />
        </View>
        
        <StyledText style={dynamicStyles.heading}>Find Your Friends</StyledText>
        <StyledText style={dynamicStyles.subheading}>
          Allow access to your contacts to quickly find and connect with friends who are already on DeezChatz.
        </StyledText>

        <View style={dynamicStyles.buttonContainer}>
          <StyledButton style={dynamicStyles.allowButton} onPress={handleAllow}>
            <StyledText style={dynamicStyles.allowButtonText}>Allow Contacts</StyledText>
          </StyledButton>
